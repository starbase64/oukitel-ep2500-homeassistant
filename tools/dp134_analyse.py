#!/usr/bin/env python3
"""
Wertet DP 134 (Betriebsstatus) aus dem Bridge-Log aus.

Fuehrt den Zustand aller Datenpunkte ueber die Zeit mit und zeigt bei jedem
Wechsel von DP 134, wie die uebrigen Werte in diesem Moment aussahen. Daraus
...
Aufruf auf dem Rechner, auf dem die Bridge laeuft:
    ...
    python3 dp134_analyse.py /tmp/ep.log
...
def main():
    ...
    if not uebergaenge:
        print("Keine Uebergaenge von DP 134 im Log gefunden.")
        print("Steht LOG_DPS auf 2? Und deckt der Zeitraum die Tests ab?")
        return

    print(f"{len(uebergaenge)} Uebergaenge von DP 134\n")
